...
/**
 * REST controller for managing {@link com.jhipster.server.domain.Answer}.
 */
@RestController
@RequestMapping("/api")
public class AnswerResource {
     ...
     */
    @GetMapping("/answers")
    public ResponseEntity<List<AnswerDTO>> getAllAnswers(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Answers");
        Page<AnswerDTO> page;
        if (eagerload) {
            page = answerService.findAllWithEagerRelationships(pageable);
        } else {
            page = answerService.findAll(pageable);
        }
